    ...
    */
    Crafty.fn = Crafty.prototype = {
        ...
        */
        ...
        isBound: function(event, callback) {
            this.each(function () {
        ...
        */
        eventCallbacksEnabled: true,
        trigger: function (event, data) {
        	if (!this.eventCallbacksEnabled)
        		return;
        	
            if (this.length === 1) {
                //find the handlers assigned to the event and entity
                //make a clone of each array, as callbacks might add/remove functions
                if (handlers[event] && handlers[event][this[0]]) {
                    var callbacks = handlers[event][this[0]].slice(0), i = 0, l = callbacks.length;
                    for (; i < l; i++) {
            ...
            for (h in hdl) {
                ...
                for (i = 0, l = hdl[h].length; i < l; i++) {
                    if (hdl[h] && hdl[h][i]) {
                        ...
                        if (entities[h] && entities[h].eventCallbacksEnabled) {
  ...
  * @see Crafty.DrawManager.draw, Crafty.DrawManager.drawAll
	...
	*/
	...
	absolutePos: function () {
		...
		if (this._parent) {
			var parentPos = this._parent.absolutePos();
			absPos._x += parentPos._x;
			absPos._y += parentPos._y;
		}
...
/**@
	...
	*/
    load: function (data, oncomplete, onprogress, onerror) {
        ...
        var i = 0, l = data.length, current, obj, total = l, j = 0, ext = "", request;
        ...
        for (; i < l; ++i) {       
            ...
            if (Crafty.support.audio && Crafty.audio.supported[ext]) {   
            	...
            	// Treat the src as an image request.
				request = new XMLHttpRequest();
				request.src = current;
				request.onreadystatechange = function() {
					if(this.readyState == 4) {
						if (this.status == 200) {
							Crafty.asset(this.src, this.responseText);
							pro.call(this);
						} else {
							err.call(this);
						}
